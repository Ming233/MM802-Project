...
import { getTweetpredictions } from './api';
...
const BitCoinUSDPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    const prices = await getTweetpredictions();
    ...
    <div className="container-fluid">
      ...
      <div className="d-flex flex-row">
        <Space size={24}>
          ...
          <Checkbox onChange={onItemChecked} name="lstm_predicted_price" checked={lstm_predicted_price}>LSTM Predict</Checkbox>
          <Checkbox onChange={onItemChecked} name="bilstm_predicted_price" checked={bilstm_predicted_price}>bilstm_predicted_price</Checkbox>
          <Checkbox onChange={onItemChecked} name="gru_predicted_price" checked={gru_predicted_price}>LSTM gru_predicted_price</Checkbox>
        </Space>
      </div>
      <div className="d-inline-block">
        ...
        >
          ...
          {bilstm_predicted_price && <Line type="monotone" dataKey="bilstm_predicted_price" stroke="red" />}
          {gru_predicted_price && <Line type="monotone" dataKey="gru_predicted_price" stroke="red" />}
        </LineChart>
      </div>
